...
class TrajectoryTransformer(nn.Module):
    def __init__(self, feature_dim=4, embed_dim=64, num_heads=4, num_layers=2, future_frames=90):
    ...
    def forward(self, x):
        ...
        for _ in range(self.future_frames):
            pred_xy = self.output_head(current_state)
            future_predictions.append(pred_xy.unsqueeze(1))
            
        out = torch.cat(future_predictions, dim=1) 
        return out
